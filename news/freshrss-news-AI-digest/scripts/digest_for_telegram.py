#!/usr/bin/env python3
"""
Conditional digest runner for Telegram delivery.
Runs the digest only if there has been NO recent CLI session activity in the last 5 minutes.
If activity cannot be determined, defaults to running (so you don't miss the digest).
Output goes to Telegram delivery.
"""
import os
import subprocess
...
def check_recent_cli_activity():
    ...
    try:
        ...
        if result.returncode != 0:
            # If we can't check activity, return False (will run digest - safe default)
            return False
        ...
        # On any error, assume inactive (will run digest)
        return False

def main():
    """Main entry point."""
    if not check_recent_cli_activity():
        # No recent activity - run the digest and output normally (for Telegram delivery)
        ...
        # Recent activity - output nothing (so origin job will handle it)
        return 0
