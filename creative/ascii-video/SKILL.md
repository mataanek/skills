...
**Important:** If the user explicitly asks for only facts and commands, suppress all narrative, commentary, and explanatory text. Provide only the essential commands and their results.
When the user requests only facts and commands with no narrative, output only the essential commands and their results. Do not add any narrative, commentary, or explanatory text. Execute the task and report success/failure with minimal detail.
Never add narrative, explanation, or commentary when the user has explicitly requested only facts and commands.
...
### Text Overlay Timing
When implementing timed text overlays, ensure the visibility function correctly ramps in and out. A common mistake is using incorrect piecewise logic that causes messages to appear prematurely or linger. Use a minimum of fade-in and fade-out factors (each ramped over 1 second) to achieve clean transitions.

### Particle System Screen Utilization
 For full-screen ASCII rain effects, verify that particles are spawned with sufficient initial vertical range and speed to traverse the entire viewport. Spawning only near the top or using insufficient speed will result in effects confined to the upper portion of the screen. Adjust base speed and spawn y-range to guarantee coverage from top to bottom.

**Note on Performance:** Increasing particle count, max_age, or spawn rate to improve screen coverage can significantly increase computation time. Balance these parameters to avoid timeouts. Consider using a smaller font size or lower resolution if necessary to maintain performance.

### Font Size Adjustment
Default font sizes may require scaling for readability at 1920x1080. Consider increasing font size by 10-20% from baseline values and validate that text remains clear against the background.

See references/session_learnings_nix_ascii_video_20260501.md for details on workspace preference, text timing fixes, screen utilization, and script generation pitfalls.
...
| `references/audio_reactive_rain_text_technique.md` | Audio-reactive rain with timed text overlay technique: aggressive flux/bass rain, message-gated backdrop, large centered text, pure black background |
| references/optimization.md | Hardware detection, quality profiles, vectorized patterns, parallel rendering, memory management, performance buffers |
| references/session_learnings_nix_ascii_video_20260502.md | Session-specific learnings from Nix's ASCII video generation (2026-05-02): facts-only output preference, custom script generation pitfalls, text overlay timing verification, particle system screen utilization, workspace preference.
...
| references/session_learnings_nix_ascii_video_20260501.md | Session-specific learnings from Nix's ASCII video generation (2026-05-01): workspace preference, text timing fixes, screen utilization, script generation pitfalls. |
| references/session_learnings_nix_ascii_video_20260501.md | Session-specific learnings from Nix's ASCII video generation (2026-05-01): workspace preference, text timing fixes, screen utilization, script generation pitfalls. |
