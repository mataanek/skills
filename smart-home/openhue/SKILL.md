...
```bash
# Linux
curl -sL https://github.com/openhue/openhue-cli/releases/latest/download/openhue_Linux_x86_64.tar.gz | tar xz -C ~/.local/bin openhue && chmod +x ~/.local/bin/openhue
...
- Hermes-specific setup: The OpenHue CLI may require setting `HOME` environment variable to locate its config (e.g., `HOME=/home/mataanek openhue get light`)
- Config is stored in `~/.openhue/config.yaml` after successful pairing
- When using OpenHue CLI via Hermes tools, ensure the skill registration script sets the HOME environment variable appropriately
