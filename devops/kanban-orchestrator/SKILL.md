...
## Best Practice: Document What You Created\n\nAfter creating Kanban tasks, it's valuable to create supporting documentation for future reference and audit trails. This is especially helpful for:\n\n1. **Complex workflows** with multiple dependencies\n2. **Long-running projects** where you need to recall what was decided\n3. **Knowledge transfer** to other team members or future sessions\n\nRecommended documentation artifacts:\n\n- **Markdown summary** (`canban_tasks.md`): Human-readable overview of all tasks, their assignees, dependencies, and verification criteria\n- **JSON export** (`tasks.json`): Machine-readable format with UUIDs, dependencies, and status for programmatic access\n- **Plan file** (`plan.json`): The original task decomposition plan that guided the creation\n- **Diagnostic references**: Session-specific troubleshooting patterns and recovery procedures\n\nExample from a recent whiskey wiki Obsidian setup:\n- Created `/home/mataanek/.hermes/hermes-agent/kanban_tasks.md` - markdown summary\n- Created `/home/mataanek/.hermes/hermes-agent/whiskey_kanban_tasks.json` - JSON format with UUIDs and dependencies\n- Used `/home/mataanek/.hermes/plans/obsidian_whiskey_setup.json` as the guiding plan\n- Added diagnostic references in `references/diagnosing_stuck_kanban_tasks.md` for troubleshooting\n\nThis documentation makes it easy to:\n- Verify that all expected tasks were created correctly\n- Check dependency mappings\n- Recreate or adjust the board if needed\n- Provide traceability for audits or reviews\n- Diagnose and recover stuck worker situations
...
**Dashboard showing generic assignees instead of team members.** If your Kanban dashboard shows generic terms like "developer", "designer", or "tester" instead of your actual team members (nix, hex, wux), it's likely because the dashboard is querying distinct assignees from tasks rather than using the `known_assignees` function that includes profiles on disk. This can cause confusion when assigning tasks. To fix this, ensure the dashboard/backend uses `kanban_db.known_assignees(conn)` which combines profiles on disk with current task assignees.

## Diagnosing stuck workers\n\nBefore attempting recovery, diagnose why a task appears stuck:\n\n1. **Check if a worker process is actually running**: Look for the worker PID in the task details. If null or pointing to a non-existent process, no work is being done.\n2. **Inspect the task workspace**: Check `/home/mataanek/.hermes/kanban/workspaces/<task_id>/` for any output, logs, or partial results.\n3. **Verify dependencies**: Ensure all parent tasks are complete if the task has dependencies.\n4. **Check gateway status**: Confirm the Hermes gateway is running and processing tasks (`ps aux | grep hermes.*gateway`).\n5. **Look for recent events**: Check the task events table for any activity beyond \"created\" and \"claimed\".\n\n## Recovering stuck workers\n\nWhen a worker profile keeps crashing, hallucinating, or getting blocked by its own mistakes (usually: wrong model, missing skill, broken credential), the kanban dashboard flags the task with a ⚠ badge and opens a **Recovery** section in the drawer. Three primary actions:\n\n1. **Reclaim** (or `hermes kanban reclaim <task_id>`) — abort the running worker immediately and reset the task to `ready`. The existing claim TTL is ~15 min; this is the fast path out.\n2. **Reassign** (or `hermes kanban reassign <task_id> <new-profile> --reclaim`) — switch the task to a different profile and let the dispatcher pick it up with a fresh worker.\n3. **Change profile model** — the dashboard prints a copy-paste hint for `hermes -p <profile> model` since profile config lives on disk; edit it in a terminal, then Reclaim to retry with the new model.\n\nHallucination warnings appear on tasks where a worker's `kanban_complete(created_cards=[...])` claim included card ids that don't exist or weren't created by the worker's profile (the gate blocks the completion), or where the free-form summary references `t_<hex>` ids that don't resolve (advisory prose scan, non-blocking). Both produce audit events that persist even after recovery actions — the trail stays for debugging.
